...
if __name__ == "__main__":
    ...
    if online == False:
        train = data.loc[data.day < 23]  # 18,19,20,21,22,23,24
        test = data.loc[data.day == 23]  # 暂时先使用第24天作为验证集
    elif online == True:
        train = data.copy()
    ...
    features = [ 'item_brand_id', 'item_city_id', 'item_price_level', 'item_sales_level',
                ...
                'cate_inter','prop_inter','second_cate','first_cate'
                ]
    target = ['is_trade']
    ...
    if online == False:
        ...
        clf.fit(train[features], train[target], feature_name=features,
                categorical_feature=['user_gender_id','user_occupation_id','item_brand_id',"first_cate"])
